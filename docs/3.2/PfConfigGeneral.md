...
Returns a template PictureFrame "configuration struct". This configuration struct can then be passed into PfCalculate() to generate various positions and sizes for each phase in GameMaker's rendering pipeline. The configuration struct is rather complex and if you're looking for easier
"quick start" behaviour than you may want to consider calling either [PfConfigPixelArt()](PfConfigPixelArt) or [PfConfigHighRes()](PfConfigHighRes) instead. They return a similar struct to PfConfigGeneral() but pre-configured for common use cases.
