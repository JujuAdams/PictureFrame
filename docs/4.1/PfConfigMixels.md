...
`PfConfigMixels(targetWidth, targetHeight, [fullscreen], [minWidth], [minHeight], [maxWidth], [maxHeight])`
...
Convenience function that returns a configuration struct set up for pixel art games that want to allow for smooth rotation and scaling of graphics (a.k.a. "mixels"). This function will set the size of the GUI layer to be the same as the size of the camera (`.guiMode` is set to `1`).
...
!> Because `PfConfigMixels()` returns a fresh struct every time it is called, you should avoid calling this function more often than is necessary.
