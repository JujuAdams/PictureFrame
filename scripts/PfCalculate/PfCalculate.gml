...
function PfCalculate(_configurationStruct, _tryResizeWindow = false, _currentFullscreen = window_get_fullscreen(), _currentWindowWidth = __PfWindowGetWidth(), _currentWindowHeight = __PfWindowGetHeight(), _currentDisplayWidth = display_get_width(), _currentDisplayHeight = display_get_height())
{
    with(_configurationStruct)
    {
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Display margins are {_displayMarginLeft}, {_displayMarginTop}, {_displayMarginRight}, {_displayMarginBottom} (size is {_displayMarginWidth} x {_displayMarginHeight})");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Desired fullscreen state is `{_fullscreen? "true" : "false"}`. \"Display has margins\" is `{_displayHasMargins? "true" : "false"}`");
        ...
        if (_fullscreen)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"We want to be in fullscreen so the window size is the same as the display ({_windowWidth} x {_windowHeight}) and we don't want to try to resize the window");
        }
        else
        {
            ...
            if (_currentFullscreen)
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"We're leaving fullscreen to go to windowed mode so we need to resize the window");
            }
            ...
            if (_tryResizeWindow)
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"Target window size is {_windowWidth} x {_windowHeight}");
            }
            else
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"Current window size is {_windowWidth} x {_windowHeight}");
            }
        }
        ...
        if (_displayHasMargins && surfaceAvoidNotch)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"We're avoiding display cutouts so the available application surface area is {_surfaceRegionWidth} x {_surfaceRegionHeight}");
        }
        else
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"We're not avoiding display cutouts so the available application surface area is {_surfaceRegionWidth} x {_surfaceRegionHeight}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera maximum size resolved to {_cameraMaxWidth} x {_cameraMaxHeight}");
        ...
        if (surfacePixelPerfect)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera-to-window scaling factor is {_targetScale} using \"scale down\" technique");
            ...
            if (viewPixelPerfect && (_targetScale > 1))
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera-to-window scaling factor rounded to {floor(_targetScale)}");
            }
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera size expanded up to the maximum and calculated as {_outCameraWidth} x {_outCameraHeight}");
        }
        else
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera-to-window scaling factor is {_targetScale} using \"similar aspect ratio\" technique");
            
            var _outCameraWidth  = floor(_surfaceRegionWidth/_targetScale);
            var _outCameraHeight = floor(_surfaceRegionHeight/_targetScale);
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera size calculated as {_outCameraWidth} x {_outCameraHeight}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Final camera size (after minimum size limit {cameraTargetWidth} x {cameraTargetHeight}) is {_outCameraWidth} x {_outCameraHeight}");
        ...
        {
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"View-to-surface scaling factor is {floor(_outViewScale)} (rounded from {_outViewScale})");
            _outViewScale = floor(_outViewScale);
        }
        else
        {
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"View-to-surface scaling factor is {_outViewScale}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Viewport / application surface size is {_outViewWidth} x {_outViewHeight}");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application-surface-to-window scale is {_surfacePostDrawScale}");
        ...
        if (surfacePixelPerfect && (_surfacePostDrawScale > 1))
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application-surface-to-window scale rounded to {_surfacePostDrawScale}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application surface render size is {_surfacePostDrawWidth} x {_surfacePostDrawHeight}");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Window overscan scale is {windowOverscanScale} which limits the surface region to {_overscanWidth} x {_overscanHeight}");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Overscan correction scale is {_overscanCorrectionScale}, application-surface-to-window scale is now {_surfacePostDrawScale}");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Final application surface render size is {_surfacePostDrawWidth} x {_surfacePostDrawHeight}");
        ...
        if (_tryResizeWindow && trimBlackBars)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Window trimmed to {_outWindowWidth} x {_outWindowHeight}");
        }
        else
        {
            var _outWindowWidth  = _windowWidth;
            var _outWindowHeight = _windowHeight;
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Window size is {_outWindowWidth}, {_outWindowHeight}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application surface centred in the window at ({_surfacePostDrawX}, {_surfacePostDrawY})");
        ...
        if (_displayHasMargins && surfaceAvoidNotch)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application surface position avoided the display cutout at ({_surfacePostDrawX}, {_surfacePostDrawY})");
        }
        ...
        if (guiWindowStretch)
        {
            ...
            if (_displayHasMargins && guiAvoidNotch)
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI respecting display cutout: {_guiRegionWidth} x {_guiRegionHeight} at position ({_outGuiX}, {_outGuiY})");
            }
            else
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI region is the entire window: {_guiRegionWidth} x {_guiRegionHeight} at position (0, 0)");
            }
        }
        else
        {
            ...
            if (_displayHasMargins && guiAvoidNotch)
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI trying to copy the application surface but dodging the display cutout: {_guiRegionWidth} x {_guiRegionHeight} at position ({_outGuiX}, {_outGuiY})");
            }
            else
            {
                ...
                if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI sharing the application surface region: {_guiRegionWidth} x {_guiRegionHeight} at position ({_outGuiX}, {_outGuiY})");
            }
        }
        
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI mode is {guiMode}");
        ...
        if (guiMode == 0)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI size is the same as the available region {_outGuiWidth} x {_outGuiHeight}");
        }
        else if (guiMode == 1)
        {
            //Use the camera size
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI size is the same as the camera {_outGuiWidth} x {_outGuiHeight}");
        }
        else if (guiMode == 2)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI size is the same as the view {_outGuiWidth} x {_outGuiHeight}");
        }
        else if (guiMode == 3)
        {
            ...
            if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI size is the same as the target GUI size {_outGuiWidth} x {_outGuiHeight}");
        }
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"GUI size after scaling factor of {guiScale} is {_outGuiWidth} x {_outGuiHeight}");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Window-to-GUI scales are ({_windowToGuiScaleX}, {_windowToGuiScaleY})");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Application surface is {_surfaceGuiWidth} x {_surfaceGuiHeight} at position ({_surfaceGuiX}, {_surfaceGuiY}) on the GUI layer");
        ...
        if (PICTURE_FRAME_VERBOSE) __PfTrace($"Camera overscan is {cameraOverscan}, view overscan is {_viewOverscan} (added to all edges)");
